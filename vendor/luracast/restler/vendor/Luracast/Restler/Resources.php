...
use Luracast\Restler\Data\RestlerString;
...
class Resources implements iUseAuthentication, iProvideMultiVersionApi
{
    ...
    public function get($id = '')
    {
        ...
        foreach ($routes as $value) {
            foreach ($value as $httpMethod => $route) {
                ...
                $fullPath = $route['url'];
                if ($fullPath !== $target && !RestlerString::beginsWith($fullPath, $target)) {
                    continue;
                }
                $fLen = strlen($fullPath);
                if ($tSlash) {
                    if ($fLen != $tLen && !RestlerString::beginsWith($fullPath, $target . '/'))
                        continue;
                } elseif ($fLen > $tLen + 1 && $fullPath{$tLen + 1} != '{' && !RestlerString::beginsWith($fullPath, '{')) {
                ...
                foreach (static::$excludedPaths as $exclude) {
                    if (empty($exclude)) {
                        if ($fullPath == $exclude)
                            continue 2;
                    } elseif (RestlerString::beginsWith($fullPath, $exclude)) {
                        continue 2;
                    }
                }
                $m = $route['metadata'];
    ...
    public function index()
    {
        ...
        foreach ($map as $path => $description) {
            if (!RestlerString::contains($path, '{')) {
                //add id
                $r->apis[] = array(
    ...
    protected function _mapResources(array $allRoutes, array &$map, $version = 1)
    {
        foreach ($allRoutes as $fullPath => $routes) {
            ...
            if ($resource == 'resources' || RestlerString::endsWith($resource, 'index'))
                continue;
